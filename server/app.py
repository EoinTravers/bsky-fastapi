...
def get_feed_skeleton():
    logger.info(f"GET get_feed_skeleton: {request.args}")
    feed = request.args.get("feed", default=None, type=str)
    algo = algos.get(feed)
    logger.info(f"feed: {feed}, algo: {algo}")
    if not algo:
    ...
    try:
        requester_did = validate_auth(request)
        logger.info(f"requester_did: {requester_did}")
    except AuthorizationError as e:
        logger.warning(f"Could not validate auth: {e}")
        pass
    ...
    logger.info(f"Response: {body.model_dump()}")
